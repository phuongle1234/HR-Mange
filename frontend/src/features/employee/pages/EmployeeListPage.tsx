...
import { setEmployeeCheckedIds } from '../../../store/employeeSelection/employeeSelectionSlice';
...
import { useCreateInvitationsMutation } from '../../invitations/hooks/useCreateInvitationsMutation';
...
export function EmployeeListPage() {
  ...
  const employeesQuery = useQuery({ queryKey: employeeQueryKeys.list(queryState), queryFn: () => employeeApiService.list(queryState) });
  const deleteEmployeeMutation = useDeleteEmployeeMutation();
  const inviteMutation = useCreateInvitationsMutation();
  ...
  function toggleRow(employee: Employee) {
    setCheckedIds((current) => current.includes(employee.id) ? current.filter((id) => id !== employee.id) : [...current, employee.id]);
  }
  ...
  function handleUpdateSelected() {
    ...
    dispatch(setEmployeeCheckedIds(checkedIds));
  ...
  async function handleConfirmDelete() {
    ...
    try {
      await employeeApiService.bulkDelete({ ids: checkedIds });
      toast.success('Selected employees deleted successfully.', { position: 'top-right' });
      setCheckedIds([]);
  ...
  async function handleInviteUser() {
    ...
    try {
      await inviteMutation.mutateAsync({ employeeIds: checkedIds });
      toast.success('Invitation emails queued successfully.', { position: 'top-right' });
    } catch (error) {
      toast.error((error as FrontendApiError).message, { position: 'top-right' });
    }
  }
  ...
  return (
    <div>
      ...
      <ContextMenu items={[
        { key: 'create', label: 'Create', onSelect: () => navigate('/employees/create') },
        { key: 'update', label: 'Update', disabled: checkedIds.length === 0, onSelect: handleUpdateSelected },
        { key: 'invite', label: 'Invite User', disabled: checkedIds.length === 0 || inviteMutation.isPending, onSelect: handleInviteUser },
        { key: 'delete', label: 'Delete', disabled: checkedIds.length === 0 || deleteEmployeeMutation.isPending, danger: true, onSelect: () => setIsDeleteConfirmOpen(true) },
      ]}>
        {({ onContextMenu }) => (
            ...
            <div className="h-[calc(100vh-290px)] overflow-auto">
              <table className="w-full text-left">
                <thead className="sticky top-0 z-10 bg-slate-50 shadow-[0_1px_0_0_rgba(226,232,240,1)]">
                  <tr>
                    <th className="w-12 px-4 py-3"><input type="checkbox" aria-label="Check all employees" checked={allVisibleChecked} onChange={toggleAllVisible} className="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500" /></th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {employeesQuery.isLoading && <tr><td colSpan={9}><LoadingState label="Loading employees..." /></td></tr>}
                  {!employeesQuery.isLoading && employeesQuery.isSuccess && employees.length === 0 && <tr><td colSpan={9}><EmptyState label="No employees found." /></td></tr>}
                  {!employeesQuery.isLoading && employeesQuery.isSuccess && employees.map((employee) => (
                    <tr key={employee.id} className="hover:bg-slate-50">
                      <td className="px-4 py-4"><input type="checkbox" aria-label={`Check ${employee.employeeCode}`} checked={checkedIds.includes(employee.id)} onChange={() => toggleRow(employee)} className="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500" /></td>
                      ...
                      <td className="px-4 py-4"><button type="button" className="text-sm font-semibold text-brand-600 hover:text-brand-700" onClick={() => navigate(`/employees/${employee.id}`)}>Detail</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            <Pagination page={meta?.page || 1} totalPages={totalPages} total={meta?.total || 0} limit={meta?.limit || limit} itemLabel="employees" onPageChange={(nextPage) => setPage(nextPage)} />
          </div>
        )}
      </ContextMenu>
      <ConfirmDialog isOpen={isDeleteConfirmOpen} title="Delete Employees" message={`Delete ${checkedIds.length} selected employee${checkedIds.length === 1 ? '' : 's'}?`} confirmLabel="Confirm delete" isConfirming={deleteEmployeeMutation.isPending} errorMessage={deleteConfirmError} onConfirm={handleConfirmDelete} onCancel={handleCancelDelete} />
    </div>
  );
}
