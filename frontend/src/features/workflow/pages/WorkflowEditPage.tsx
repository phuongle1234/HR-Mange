...
import { useUpdateWorkflowMutation } from '../hooks/useUpdateWorkflowMutation';
import { useWorkflowDetailQuery } from '../hooks/useWorkflowDetailQuery';
import type { Workflow, WorkflowFormField, WorkflowFormFieldType, WorkflowStatus, WorkflowStep } from '../types/workflow.types';
...
const workflowEditTableSchema = z.object({
...
type WorkflowEditTableValues = z.infer<typeof workflowEditTableSchema>;
type WorkflowFieldRow = WorkflowEditTableValues['fields'][number];
type WorkflowStepRow = WorkflowEditTableValues['steps'][number];
...
function formatOptionsText(field: WorkflowFormField) {
...
function mapWorkflowToFormValues(workflow: Workflow): WorkflowEditTableValues {
  return {
    code: workflow.code,
    name: workflow.name,
    description: workflow.description ?? '',
    status: workflow.status,
    fields: workflow.formSchema.fields.map((field) => ({ rowId: crypto.randomUUID(), key: field.key, label: field.label, type: field.type, required: field.required, placeholder: field.placeholder ?? '', optionsText: formatOptionsText(field) })),
    steps: (workflow.steps && workflow.steps.length > 0 ? workflow.steps : []).map((step) => ({ rowId: step.id || crypto.randomUUID(), name: step.name, organizationTypeId: step.organizationTypeId })),
  };
}

function buildStepPreview(steps: WorkflowStepRow[], organizationTypes: Array<{ id: string; name: string }>, workflowId = ''): WorkflowStep[] {
...
export function WorkflowEditPage() {
  ...
  const [pendingValues, setPendingValues] = useState<WorkflowEditTableValues | null>(null);
  const [submitError, setSubmitError] = useState<string | null>(null);
  const { getGridInputProps: getFieldGridInputProps } = useGridInputNavigation({ gridName: 'workflow-edit-fields' });
  const { getGridInputProps: getStepGridInputProps } = useGridInputNavigation({ gridName: 'workflow-edit-steps' });
  const { control, register, handleSubmit, watch, reset, formState: { errors, isValid }, setError } = useForm<WorkflowEditTableValues>({ resolver: zodResolver(workflowEditTableSchema), mode: 'onChange', defaultValues: { code: '', name: '', description: '', status: 'DRAFT', fields: [createFieldRow()], steps: [createStepRow()] } });
  ...
  const stepPreview = useMemo(() => buildStepPreview(watchedSteps, organizationTypes, id ?? ''), [id, organizationTypes, watchedSteps]);
  ...
  const isSaving = updateMutation.isPending || replaceStepsMutation.isPending;

  useEffect(() => {
    if (!workflow) return;

    const nextValues = mapWorkflowToFormValues(workflow);
    reset({ ...nextValues, fields: nextValues.fields.length > 0 ? nextValues.fields : [createFieldRow()], steps: nextValues.steps.length > 0 ? nextValues.steps : [createStepRow()] });
    setCheckedFieldRows([]);
    setCheckedStepRows([]);
  }, [reset, workflow]);
  ...
  function toggleFieldRow(rowId: string) {
    setCheckedFieldRows((current) => current.includes(rowId) ? current.filter((fieldId) => fieldId !== rowId) : [...current, rowId]);
  }

  function toggleStepRow(rowId: string) {
    setCheckedStepRows((current) => current.includes(rowId) ? current.filter((stepId) => stepId !== rowId) : [...current, rowId]);
  }
  ...
  function onValid(values: WorkflowEditTableValues) {
    setSubmitError(null);
  ...
  async function handleConfirmSubmit() {
    ...
    try {
      ...
      setSubmitError(apiError.message || 'Unable to update workflow.');
      if (!hasFieldErrors) {
        setError('root', { message: apiError.message || 'Unable to update workflow.' });
      }
    }
  }
  ...
  if (!workflow) {
    return <ErrorState message="Workflow not found." onRetry={() => workflowQuery.refetch()} />;
  }
  ...
  return (
    <div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit(onValid)} onContextMenu={onContextMenu} noValidate>
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-500">Workflow definition</p>
                  <h2 className="text-2xl font-black text-slate-900">{workflow.name}</h2>
                </div>
      ...
      <ConfirmDialog isOpen={Boolean(pendingValues)} title="Confirm Update Workflow" message="Review the form schema and approval steps before updating this workflow." confirmLabel="Confirm submit" isConfirming={isSaving} errorMessage={submitError ?? errors.root?.message} onConfirm={handleConfirmSubmit} onCancel={() => setPendingValues(null)} />
    </div>
  );
}
