...
export function MyRequestsPage() {
  ...
  const requestsQuery = useWorkflowRequestListQuery({ scope: 'mine', page: queryState.page, limit: queryState.limit, search: useDebounce(queryState.search, SEARCH_DEBOUNCE_MS), status: status || undefined, sortBy: queryState.sortBy, sortOrder: queryState.sortOrder });
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <SearchAndFilterBar searchValue={queryState.search} onSearchChange={queryState.handleSearchChange} searchPlaceholder="Search my requests" createLabel="New Request" onCreate={() => navigate('/workflow-requests/new')} limitValue={queryState.limit} onLimitChange={queryState.handleLimitChange}>
          <select aria-label="Filter my requests by status" value={status} onChange={(event) => handleStatusChange(event.target.value as WorkflowRequestStatus | '')} className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-100">
            <option value="">All statuses</option>
            {WORKFLOW_REQUEST_STATUS_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}
          </select>
        </SearchAndFilterBar>
        {requestsQuery.isError && <ErrorState message="Unable to load requests." onRetry={() => requestsQuery.refetch()} />}
        ...
        <div className="h-[calc(100vh-290px)] overflow-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {requestsQuery.isLoading && <tr><td colSpan={4}><LoadingState label="Loading requests..." /></td></tr>}
              {!requestsQuery.isLoading && requestsQuery.isSuccess && items.length === 0 && <tr><td colSpan={4}><EmptyState label="No requests found." /></td></tr>}
              {!requestsQuery.isLoading && requestsQuery.isSuccess && items.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3"><button type="button" onClick={() => navigate(`/workflow-requests/${item.id}`)} className="font-black text-brand-700">{item.workflow?.name ?? 'Workflow'}</button></td>
        ...
        <Pagination currentPage={meta?.page || queryState.page} totalPages={totalPages} total={meta?.total || 0} limit={meta?.limit || queryState.limit} itemLabel="requests" onPageChange={queryState.setPage} />
      </div>
    </div>
  );
}
