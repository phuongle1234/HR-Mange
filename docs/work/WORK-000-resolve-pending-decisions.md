...
| 4 | Auth token transport | **Bearer token in response body.** `POST /api/auth/login` returns the access token in the JSON body; the frontend stores it (in memory/Redux, persisted client-side for page-reload continuity) and sends it via the `Authorization: Bearer <token>` header. No auth cookie is sent to the API. <br><br>*Amended 2026-08-28:* the client-side persistence medium changed from `localStorage` to an expiring browser cookie (expiry derived from the JWT's own `exp`), because `localStorage` has no expiry and outlived the token. This does **not** reverse "no auth cookie" — the cookie is JS-written local storage only; `withCredentials` stays `false` and the backend still authenticates via the `Authorization` header. See `FRONTEND-AUTH-PROVIDER` → Token Storage. |
